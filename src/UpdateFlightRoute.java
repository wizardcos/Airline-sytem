...
public class UpdateFlightRoute extends JFrame 
{
	...
	public UpdateFlightRoute(int index)
	{
		...
		JLabel lblAddNewFlight = new JLabel("Edit The Selected Route");
		...
		JButton btnaUpdate = new JButton("Update");
		btnaUpdate.addMouseListener(new MouseAdapter() {
			@Override
					...
					!fieldbusinessSeats.getText().equals(""))
						{
							if (validSeats(fieldeconomySeats.getText())==true && validSeats(fieldbusinessSeats.getText())==true)
							{
								...
								if ((eco>=0 && eco<=50) && (business>=0 && business<=10))
								{
									if(validFlight()==true)
									{
										...
										tempobject.setFlightId(fieldflightId.getText());
										tempobject.setDepartureAirport(fielddepartureAirport.getText());
										tempobject.setLandingAirport(fieldlandingAirport.getText());
										tempobject.setDepartureTime(fielddepartureTime.getText());
										tempobject.setLandingTime(fieldlandingTime.getText());
										tempobject.setSeatsEconomy(eco);
										tempobject.setSeatsBusiness(business);
											...
											new JOptionPane();
											JOptionPane.showMessageDialog(panel,"Successfully Updated","Successful",JOptionPane.INFORMATION_MESSAGE);	
									}
									else
									{
										new JOptionPane();
										JOptionPane.showMessageDialog(panel,"Five Flights can not Land at a time at same Airport","Failed",JOptionPane.INFORMATION_MESSAGE);	
									}
								}
								else
								{
									new JOptionPane();
									JOptionPane.showMessageDialog(panel,"Invalid Numbers for Seats","Failed",JOptionPane.INFORMATION_MESSAGE);
								}
							...
							{
									new JOptionPane();
									JOptionPane.showMessageDialog(panel,"Please Enter Only positive Integers for Seats!","Error",JOptionPane.INFORMATION_MESSAGE);
							}
						}
				else
				{
		...
		btnaUpdate.setForeground(Color.RED);
		btnaUpdate.setFont(new Font("Tahoma", Font.PLAIN, 15));
		btnaUpdate.setBounds(161, 395, 112, 35);
	...
	//Function to check that more than 5 flights do not land at same time at same airport
